...
"""
Calibration tool for PixSWAB model based on the adaptation of the 
...
To run the calibaration tool, prepare the calibration settings file and the model config file template run in the batch file run_cal.bat.
...
path_subcatch = os.path.join(rootbasin, parser.get('Path','SubCatchmentPath'))

# Load xml and .bat template files
runmodel = os.path.splitext(os.path.join(root,modeltemplate,RunModel_template))[0]
# print(root,modeltemplate, RunModel_template)
...
template_bat = f.read()
f.close()
f = open(os.path.join(root,modeltemplate,ModelSettings_template),"r")
template_xml = f.read()
f.close()
...
def RunModel(Individual):
	...
	# print(SubCatchmentPath,path_subcatch,directory_run,'\n', os.path.abspath(directory_run))

	template_xml_new = template_xml
	template_xml_new = template_xml_new.replace("%root", root)
	# template_xml_new = template_xml_new.replace("%pathcurrent", root)
	for ii in range(0,len(ParamRanges)-1):
		template_xml_new = template_xml_new.replace("%"+ParamRanges.index[ii],str(Parameters[ii]))
	# replace output directory
	# directory_run2 =  os.path.join(root, parser.get('Path','SubCatchmentPath'), run_rand_id)
	...
	simulated_streamflow = pandas.read_csv(Qsim_tss,sep=r",",index_col=0)
	simulated_streamflow['discharge'][simulated_streamflow['discharge']==1e31] = np.nan
	
	simulated_streamflow = simulated_streamflow[simulated_streamflow.index.isin(observed_streamflow.index)]
	
    # make the length of the simulated and observed discharge the same

	if len(observed_streamflow) != len(simulated_streamflow['discharge']):
		raise Exception("run_rand_id: " + str(
			...
			len(observed_streamflow)) + " and " + str(len(simulated_streamflow['discharge'])) + " elements, respectively)")

	#Qobs = observed_streamflow[Cal_Start:Cal_End].values+0.001
	#Qobs = observed_streamflow
	q1 = simulated_streamflow['discharge'].values+0.0001
...
if __name__ == "__main__":
	...
	if bestrun:
		...
		simulated_streamflow = pandas.read_csv(Qsim_tss,sep=r",",index_col=0)
		simulated_streamflow['discharge'][simulated_streamflow['discharge']==1e31] = np.nan
		Qsim = simulated_streamflow['discharge'].values
