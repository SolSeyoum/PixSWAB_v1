...
def prepare_inputs(iniFile):
    ...
    bsahpe = os.path.join(root, parser.get('BASIN_SHAPE', 'basinShape'))
    return pixswab_inputs, pixswab_parameters, log_file_path, dir_out, encoding_output, bsahpe
...
#     print('Parameters d_bf: {0}, dp: {1}, rcaf: {2}'.format(pixswab_parameters['baseFlowReces'], 
#                                                             pixswab_parameters['deepPerc'],
#                                                             rcaf))
...
def compute_discharge(result, basin_shp, dir_out, log_file_path):
    ...
    for r in result:
        if(r is not None):
            if((r.name == 'Surface_runoff')|(r.name == 'Base_flow')):
                dss.append(r)
    # print(dss)
    ...
    df['discharge'] = df['Base_flow']+df['Surface_runoff']
    # print(df)
    # file_name = os.path.join(dir_out,'discharge_monthavg.csv')
    # df.to_csv(file_name, sep=',')
    # log(log_file_path, ' Discharge timeseries data saved to {}.'.format(file_name), logging.INFO)
...
#     pixswab_inputs, pixswab_parameters, log_file_path, out_dir, encoding_output, bsahpe = prepare_inputs(iniFile)
#     result = run_pixswab(pixswab_inputs, pixswab_parameters)
#     compute_discharge(result, bsahpe, out_dir, log_file_path)
